...
var smartblocks = {
    ...
    createBlock: function (name) {
        ...
        fs.mkdir(blockpath, function () {
            ncp(path.join(__dirname, 'project_structure', 'new_block', 'backend'), path(blockpath, 'backend'));
            ncp(path.join(__dirname, 'project_structure', 'new_block', 'frontend'), path(blockpath, 'frontend'));
            fs.readFile(path.join(__dirname, 'project_structure', 'new_block', 'descriptor_template'), 'UTF-8', function (err, data) {
                ...
                fs.writeFile(path.join(blockpath, 'descriptor.js'), html, function (err) {
                    if (err) throw err;
                ...
                fs.mkdir(apppath, function () {
                    ...
                    fs.mkdir(path.join(apppath, 'config'), function () {
                        fs.readFile(path.join(__dirname, 'project_structure', 'config', 'index.std.js'), {}, function (err, data) {
                            console.log(path.join(__dirname, 'project_structure', 'config', 'index.std.js'), data);
                            fs.writeFile(path.join(apppath, 'config', 'index.js'), data, {}, function (err) {
                                if (err) console.log("An error occured while creating the config file");
                            });
                        });
                    });
