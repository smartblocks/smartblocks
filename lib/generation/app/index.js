...
module.exports = {
    run: function () {
        ...
        inquirer.prompt(prompts, function (answers) {
            ...
            var view_name = '';
            for (var k = 0; k < answers.app_name.length; k++) {
                if (answers.app_name[k] === '_') {
                    continue;
                } else if (answers.app_name[k - 1] && answers.app_name[k - 1] === '_' || k == 0) {
                    view_name += answers.app_name[k].toUpperCase();
                } else {
                    view_name += answers.app_name[k];
                }
            ...
            fs.mkdir(path.join(block_path, 'frontend', 'apps', answers.app_name), function (err) {
                if (!err) {
                    async.parallel([
                        function (cb) {
                            ...
                            fs.readFile(path.join(block_path, 'descriptor.json'), 'UTF-8', function (err, data) {
                                ...
                                descriptor.apps.push({
                                    ...
                                    routing: answers.block_name + '/' + answers.app_name
                                });
                        ...
                        function (cb) {
                            ...
                            fs.mkdir(path.join(block_path, 'frontend', 'apps', answers.app_name, 'style'), function (err) {
                                if (err) {
                                    ...
                                    fs.writeFile(path.join(block_path, 'frontend', 'apps', answers.app_name, 'style', 'main.less'), '.' + answers.app_name + '_view {}', function (err) {
                                        if (!err) {
                                            created.style = true;
                                        }
                                        cb();
                        ...
                        /**
                         * This function adds code to launch the first view of the app
                         * in the block's index.js file.
                         * @param cb
                         */
                            function (cb) {
                            fs.readFile(path.join(block_path, 'frontend', 'index.js'), 'UTF-8', function (err, data) {
                                if (err) {
                                    ...
                                    var updated = data.replace(os.EOL + '        //new_apps - don\'t remove that comment',
                                        ',' + os.EOL + '        launch_' + answers.app_name + ': function (app) {' + os.EOL +
                                            '            ' + 'var view = new ' + view_name + '();' + os.EOL +
                                            '            ' + 'SmartBlocks.Methods.render(view.$el);' + os.EOL +
                                            '            ' + 'view.init(app);' + os.EOL +
                                            '        }' +
                                            os.EOL + '        //new_apps - don\'t remove that comment');

                                    var updated = updated.replace(os.EOL + '    //new_apps_deps - don\'t remove that comment', ',' + os.EOL + '    ' +
                                        '\'./apps/' + answers.app_name + '/views/main\'' + os.EOL + '    //new_apps_deps - don\'t remove that comment');

                                    var updated = updated.replace('/*new_deps*/', ', ' + view_name + '/*new_deps*/');
                        ...
                        function (cb) {
                            fs.readFile(path.join(block_path, 'frontend', 'index.less'), 'UTF-8', function (err, data) {
                                if (!err) {

                                    var updated = data + os.EOL + '@import "apps/' + answers.app_name + '/style/main";';
                                    if (updated !== data) {
                                        fs.writeFile(path.join(block_path, 'frontend', 'index.less'), updated, function (err) {
                                            if (!err) {
                        ...
                        if (created.style_link) {
                            console.log('The import statement in the index.less file was successfully added.'.green);
                        } else {
                            console.log('The import statement in the index.less file could not be added'.red);
                        }
